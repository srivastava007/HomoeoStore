...
export default function PackingMaster() {
  ...
  async function handleSave(e) {
    ...
    setError('')
    try {
      if (!name.trim()) throw new Error("Packing name is required")
      if (editingId) {
        await window.api.updatePacking({ id: editingId, name: name.trim() })
      } else {
        await window.api.addPacking({ name: name.trim() })
      }
      ...
      if (err.message.includes('UNIQUE')) {
        setError('A Packing with this name already exists.')
      } else {
  ...
  async function handleDelete(id) {
    showConfirm('Delete Packing', 'Are you sure you want to delete this packing? This action cannot be undone.', async () => {
      try {
        await window.api.deletePacking(id)
        load()
      } catch (err) {
        showAlert('Error', 'Failed to delete packing.')
      }
    })
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', height: '100%', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: '800', color: '#0f172a', margin: '0 0 4px 0', letterSpacing: '-0.5px' }}>Packing Master</h1>
          <p style={{ fontSize: '14px', color: '#64748b', margin: 0 }}>Manage medicine packings (e.g., 1 DRAM, 10 ML, 30 ML)</p>
        </div>
        <button 
      ...
      <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #e2e8f0', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
        <div style={{ flex: 1, padding: '32px', overflowY: 'auto', background: '#f8fafc' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '20px' }}>
            {packings.map(p => (
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', border: '1px solid #e2e8f0', borderRadius: '16px', background: '#fff', transition: 'all 0.2s' }} className="packing-card">
                <span style={{ fontSize: '16px', fontWeight: '800', color: '#0f172a', letterSpacing: '0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '18px' }}>📦</span> {p.name}
                </span>
            ...
            {packings.length === 0 && (
              <div style={{ gridColumn: '1 / -1', padding: '60px', textAlign: 'center', color: '#94a3b8', fontSize: '15px' }}>
                ...
                No packings added yet. Click "+ New Packing" to start.
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: '#fff', borderRadius: '24px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 40px -10px rgba(0,0,0,0.2)', border: '1px solid #e2e8f0', overflow: 'hidden', animation: 'scaleIn 0.2s ease-out' }}>
            ...
            <div style={{ padding: '24px', background: 'linear-gradient(to right, #0f172a, #1e293b)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800', color: '#f8fafc', letterSpacing: '-0.5px' }}>{editingId ? 'Edit Packing' : 'Add New Packing'}</h2>
            ...
            <form onSubmit={handleSave} style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '12px', fontWeight: '700', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Packing Name <span style={{ color: '#ef4444' }}>*</span></label>
                <input 
                  type="text" 
                  value={name} 
                  onChange={e => setName(e.target.value)} 
                  placeholder="e.g. 100 ML, 1 DRAM"
