...
export default function CategoryMaster() {
  ...
  async function handleSave(e) {
    ...
    setError('')
    try {
      if (!name.trim()) throw new Error("Type Name is required")
      if (editingId) {
        await window.api.updateCategory({ id: editingId, name: name.trim() })
      } else {
        await window.api.addCategory({ name: name.trim() })
      }
  ...
  async function handleDelete(id) {
    showConfirm('Delete Type', 'Are you sure you want to delete this type? This action cannot be undone.', async () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', height: '100%', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: '800', color: '#0f172a', margin: '0 0 4px 0', letterSpacing: '-0.5px' }}>Category Master</h1>
          <p style={{ fontSize: '14px', color: '#64748b', margin: 0 }}>Manage medicine categories (e.g., Dilution, Mother Tincture, Syrup)</p>
        </div>
        <button 
      ...
      <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #e2e8f0', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
        <div style={{ flex: 1, padding: '32px', overflowY: 'auto', background: '#f8fafc' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            {categories.map(cat => (
              <div key={cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', border: '1px solid #e2e8f0', borderRadius: '16px', background: '#fff', transition: 'all 0.2s' }} className="type-card">
                <span style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '18px' }}>💊</span> {cat.name}
                </span>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => { setEditingId(cat.id); setName(cat.name); setIsModalOpen(true); }} style={{ background: '#f1f5f9', color: '#3b82f6', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s' }} className="btn-edit">Edit</button>
                  <button onClick={() => handleDelete(cat.id)} style={{ background: '#fef2f2', color: '#ef4444', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s' }} className="btn-delete">Delete</button>
                </div>
              </div>
            ))}
            ...
            {categories.length === 0 && (
              <div style={{ gridColumn: '1 / -1', padding: '60px', textAlign: 'center', color: '#94a3b8', fontSize: '15px' }}>
                ...
                No categories added yet. Click "+ New Category" to start.
              </div>
            )}
          </div>
        </div>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: '#fff', borderRadius: '24px', width: '100%', maxWidth: '450px', boxShadow: '0 20px 40px -10px rgba(0,0,0,0.2)', border: '1px solid #e2e8f0', overflow: 'hidden', animation: 'scaleIn 0.2s ease-out' }}>
            ...
            <div style={{ padding: '24px', background: 'linear-gradient(to right, #0f172a, #1e293b)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '800', color: '#f8fafc', letterSpacing: '-0.5px' }}>{editingId ? 'Edit Category' : 'Add New Category'}</h2>
            ...
            <form onSubmit={handleSave} style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '12px', fontWeight: '700', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Category Name <span style={{ color: '#ef4444' }}>*</span></label>
                <input 
                  type="text" 
                  value={name} 
                  onChange={e => setName(e.target.value)} 
                  placeholder="e.g. Dilution, Ointment, Syrup"
              ...
              <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                <button type="submit" style={{ padding: '12px 32px', borderRadius: '12px', border: 'none', background: '#10b981', color: '#fff', fontWeight: '800', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontSize: '14px', boxShadow: '0 4px 14px rgba(16, 185, 129, 0.3)', transition: 'all 0.2s' }} className="btn-hover-primary">{editingId ? 'Update Category' : 'Save Category'}</button>
              </div>
            </form>
          </div>
        </div>
