  ...
  return (
    <div style={{ height: '100vh', width: '100vw', position: 'relative', overflow: 'hidden', fontFamily: 'Outfit, sans-serif', background: '#ffffff' }}>
      ...
      <div style={{ position: 'absolute', top: '24px', left: '24px', zIndex: 100, WebkitAppRegion: 'no-drag' }}>
        ...
        {showNetworkMenu && (
          <div className="glass-panel" style={{ position: 'absolute', top: '100%', left: 0, marginTop: '8px', borderRadius: '16px', padding: '16px', minWidth: '240px', background: '#ffffff', color: '#1e293b', border: '1px solid #e2e8f0', boxShadow: '0 10px 25px rgba(0,0,0,0.08)' }}>
          ...
          {(!role && !isPinSetup) ? (
            ...
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <h2 style={{ fontSize: '22px', fontWeight: '700', color: '#1e293b', margin: '0 0 8px 0' }}>{isPinSetup ? 'Set Master PIN' : (role === 'admin' ? 'Admin Login' : 'Cashier Login')}</h2>
                <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '32px' }}>{isPinSetup ? 'Create a new 4+ digit PIN for Admin access.' : 'Please enter your PIN to continue.'}</p>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <button type="submit" style={{ padding: '16px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: '#fff', border: 'none', borderRadius: '16px', fontSize: '16px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3)', transition: 'all 0.2s' }}>
                    {isPinSetup ? 'Set PIN & Login' : 'Unlock Dashboard'}
                  </button>
                </form>
              </div>
            </div>
