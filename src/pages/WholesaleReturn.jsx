...
export default function WholesaleReturn() {
    ...
    async function loadHistory() {
        try {
            const data = await window.api.getWholesaleReturns()
            setHistory(data)
        } catch (err) {
            console.error(err)
        }
    ...
    async function handleSearch(e) {
        ...
        setError('')
        try {
            const data = await window.api.getWholesaleBillForReturn(billNumber.trim())
            if (!data) {
    ...
    async function executeSubmit() {
        ...
        try {
            const returnData = {
                ...
                total_amount: amount,
                reason: reason || 'Wholesale Return',
            ...
            await window.api.createWholesaleReturn(returnData)
            showToast('Wholesale Return (Credit Note) created successfully! Stock updated.', 'success')
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', height: '100%', position: 'relative', fontFamily: 'Outfit, sans-serif' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <div>
                    <h1 style={{ fontSize: '26px', fontWeight: '800', color: '#0f2d1f', margin: 0, letterSpacing: '-0.5px' }}>Wholesale Return</h1>
                        ...
                        <div style={{ ...sectionStyle, flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 0 }}>
                            ...
                            <div style={{ padding: '20px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{ display: 'flex', gap: '24px' }}>
                                    <div style={{ flex: 1 }}>
                                        <p style={{ fontSize: '12px', color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 4px 0' }}>Party</p>
                                        <p style={{ fontSize: '18px', fontWeight: '700', color: '#0f172a', margin: 0 }}>{billData.party_name || 'B2B Party'}</p>
                                        <p style={{ fontSize: '13px', color: '#64748b', margin: '4px 0 0 0' }}>{billData.party_phone || 'No phone provided'}</p>
                                    </div>
                            ...
                            <tbody>
                                {history.length === 0 ? (
                                    ...
                                    <tr key={h.id} className="table-row">
                                        ...
                                        <td style={{ ...tableCellStyle }}>{h.party_name || <span style={{ color: '#94a3b8', fontStyle: 'italic' }}>Walk-in</span>}</td>
